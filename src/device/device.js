		...
		return {
			...
			getMatchMap: function (u) {
				return {
					// 内核
					Trident:
						u.indexOf('Trident') > -1 || u.indexOf('NET CLR') > -1,
					...
					Firefox:
						u.indexOf('Firefox') > -1 || u.indexOf('FxiOS') > -1,
					...
					360:
						u.indexOf('QihooBrowser') > -1 ||
						u.indexOf('QHBrowser') > -1,
					...
					'Windows Phone':
						u.indexOf('IEMobile') > -1 ||
						u.indexOf('Windows Phone') > -1,
					BlackBerry:
						u.indexOf('BlackBerry') > -1 || u.indexOf('RIM') > -1,
					...
					Mobile:
						...
						u.indexOf('480') > -1,
					Tablet:
						u.indexOf('Tablet') > -1 || u.indexOf('Nexus 7') > -1,
			...
			matchInfoMap: function () {
				const _this = this
				var UA = VariableLibrary.navigator.userAgent || {}
				var match = MethodLibrary.getMatchMap(UA)
				for (var info in VariableLibrary.infoMap) {
					for (
						var i = 0;
						i < VariableLibrary.infoMap[info].length;
						i++
					) {
						var value = VariableLibrary.infoMap[info][i]
						if (match[value]) {
							_this[info] = value
						}
					}
			...
			getBrowserInfo: function () {
				...
				if (is360) {
					if (_mime('type', 'application/gameplugin')) {
						...
						typeof VariableLibrary.navigator['connection'][
							'saveData'
						] == 'undefined'
					) {
						match['360SE'] = true
				...
				if (
					_this.browser == 'Opera' &&
					parseInt(_this.browserVersion) > 12
				) {
					_this.engine = 'Blink'
				}
